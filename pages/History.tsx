...
const History: React.FC = () => {
  ...
  if (activityLog.length === 0) {
    return (
        <div className="text-center py-10">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{t('history.empty.title')}</h2>
            <p className="text-gray-600 dark:text-slate-400">{t('history.empty.subtitle')}</p>
        </div>
    )
  }
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">{t('history.title')}</h2>
      <motion.div 
        ...
        animate="visible"
      >
        <div className="absolute left-6 top-2 bottom-2 w-0.5 bg-gray-200 dark:bg-white/10 sm:left-7"></div>
        {activityLog.map(log => {
          ...
          return (
            ...
            >
                <div className={`absolute left-0 top-1.5 p-2 bg-gray-50 dark:bg-[#0B1120] rounded-full ${color} ring-4 ring-gray-50 dark:ring-[#0B1120]`}>
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <div className="flex-1 bg-white dark:bg-white/5 backdrop-blur-lg border border-gray-200 dark:border-white/10 p-4 rounded-lg w-full">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-start">
                        <div className="flex-1 mb-1 sm:mb-0">
                            <p className="font-semibold text-gray-900 dark:text-white">{title}</p>
                            {log.details && (
                            <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{t('history.details')} : {log.details}</p>
                            )}
                        </div>
                        <p className="text-xs text-gray-500 dark:text-slate-500 whitespace-nowrap sm:ms-4 flex-shrink-0">{formatTimestamp(log.createdAt)}</p>
                    </div>
                </div>
            </motion.div>
          );
