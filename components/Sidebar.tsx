...
const Sidebar: React.FC<{ isOpen: boolean; toggle: () => void }> = ({ isOpen, toggle }) => {
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full bg-dark text-white w-64 p-4 z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="flex items-center mb-10 px-2">
            ...
            <h1 className="text-2xl font-bold ml-3">Chez Hugo</h1>
        </div>
        <nav className="flex flex-col space-y-2">
            ...
            <NavItem to="/sales" icon={ShoppingCartIcon} label="Ventes" onClick={window.innerWidth < 768 ? toggle : undefined} />
