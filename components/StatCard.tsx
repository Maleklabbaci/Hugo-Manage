...
const StatCard: React.FC<StatCardProps> = ({ icon: Icon, title, value, description }) => {
  return (
    ...
    >
      ...
      <div>
        <p className="text-xs md:text-sm text-gray-600 dark:text-slate-400 truncate">{title}</p>
        <p className="text-lg md:text-2xl font-bold text-gray-900 dark:text-white">{value}</p>
        {description && <p className="hidden md:block text-xs text-gray-500 dark:text-slate-500">{description}</p>}
      </div>
    </motion.div>
  );
};
